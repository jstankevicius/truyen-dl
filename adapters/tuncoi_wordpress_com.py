...
class TuncoiAdapter(URLAdapter):
    """Adapter for tuncoi.wordpress.com serialized novels."""
    ...
    def _chapter_sort_key(self, text: str, href: str):
        ...
        match = re.search(r"phiên ngoại\s*(\d+)", lower)
        if match:
            return (1, int(match.group(1)), 0)

        match = re.search(r"me-muoi-chuong-(\d+)", href_lower)
        if match:
            return (0, int(match.group(1)), 0)

        match = re.search(r"me-muoi-phien-ngoai-(\d+)", href_lower)
        if match:
            return (1, int(match.group(1)), 0)
    ...
    def _looks_like_chapter_link(self, text: str, href: str) -> bool:
        ...
        return bool(
            ...
            or re.search(r"phiên ngoại\s*\d+", lower)
            or "me-muoi-chuong-" in href_lower
            or "me-muoi-phien-ngoai-" in href_lower
        )
    ...
    def get_chapter_info(self, url: str, index: int, title: str) -> ChapterInfo:
        try:
            ...
            started = False
            for block in entry.find_all(["p", "h5"]):
                text = self._normalize_text(block.get_text(" ", strip=True))
                if not text:
                    continue

                lower = text.lower()
                if lower.startswith(("chương sau", "previous post", "next post")):
                    break
