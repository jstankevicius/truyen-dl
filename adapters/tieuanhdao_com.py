...
class TieuanhdaoAdapter(URLAdapter):
    """Adapter for tieuanhdao.com serialized novels."""
    ...
    def _strip_book_prefix(self, title: str) -> str:
        cleaned = self._normalize_text(title)
        cleaned = re.sub(r"^Protected:\s*", "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\s*[-|]\s*Tiểu Anh Đào\s*$", "", cleaned, flags=re.IGNORECASE)
        return cleaned
    ...
    def _looks_like_chapter_link(self, text: str, href: str) -> bool:
        ...
        return bool(
            ...
            or re.search(r"ngoại truyện\s*\d+", lower)
            or re.search(r"protected:\s*ngày mai vẫn còn yêu em – chương\s*\d+", lower)
            or "ngay-mai-van-yeu-em-chuong-" in href_lower
            or "ngay-mai-van-con-yeu-em-chuong-" in href_lower
            or "ngay-mai-van-yeu-em-ngoai-truyen-" in href_lower
        )
    ...
    def _chapter_sort_key(self, text: str, href: str):
        ...
        match = re.search(r"protected:\s*ngày mai vẫn còn yêu em – chương\s*(\d+)", lower)
        if match:
            return (0, int(match.group(1)))

        match = re.search(r"ngay-mai-van-yeu-em-chuong-(\d+)", href_lower)
        if match:
            return (0, int(match.group(1)))

        match = re.search(r"ngay-mai-van-con-yeu-em-chuong-(\d+)", href_lower)
        if match:
            return (0, int(match.group(1)))

        match = re.search(r"ngay-mai-van-yeu-em-ngoai-truyen-(\d+)", href_lower)
        if match:
            return (1, int(match.group(1)))
    ...
    def get_book_title(self, url: str) -> str:
        ...
        if og_title and og_title.get("content"):
            return self._strip_book_prefix(og_title["content"])

        title_elem = soup.find(attrs={"class": "entry-title"})
        if title_elem:
            return self._strip_book_prefix(title_elem.get_text(" ", strip=True))

        if soup.title and soup.title.string:
            return self._strip_book_prefix(soup.title.string)
    ...
    def get_chapter_links_and_titles(self, url: str) -> list[tuple[str, str]]:
        ...
        chapter_map: dict[tuple[int, int], tuple[str, str]] = {}
    ...
    def get_chapter_info(self, url: str, index: int, title: str) -> ChapterInfo:
        try:
            ...
            if not chapter_title:
                title_elem = soup.find(attrs={"class": "entry-title"})
                if title_elem:
                    chapter_title = self._strip_book_prefix(title_elem.get_text(" ", strip=True))
                else:
                    chapter_title = f"Chương {index + 1}"

            page_text = self._normalize_text(entry.get_text(" ", strip=True))
            if "password-protected" in page_text.lower():
                print(f"Warning: protected chapter, no body extracted: {url}")
                return ChapterInfo(title=chapter_title, body=[], index=index)
            ...
            for block in entry.find_all(["p", "blockquote"]):
                ...
                if lower.startswith(("quyển ", "chương ", "dịch:", "dịch :")) and not started:
                    continue

                if lower.startswith(("tác giả:", "chuyển ngữ:", "editor:", "độ dài:", "thể loại:", "nhân vật:", "giới thiệu", "mục lục")) and not started:
                    continue
